...
import { ArrowRight, CheckCircle2, Activity, DollarSign, PieChart, BarChart3, Sun, Moon, Stethoscope } from "lucide-react";
...
export default function RxFinance() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white">
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <div className="flex flex-col items-start gap-1">
                  ...
                  <span className="text-xs text-muted-foreground pl-1">$9.99/mo after trial</span>
                </div>
      ...
      {/* ── Comparison Table (before features) ───────────── */}
      ...
      <section id="features" className="py-24 md:py-32 bg-gray-50">
        <div className="container mx-auto px-4 md:px-6">
          ...
          <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-4">
            {[
              { icon: DollarSign, color: "primary", title: "Income Tracking", desc: "Effortlessly track 1099 payments from multiple facilities and identify missing payments." },
              { icon: PieChart,   color: "accent",   title: "Solo 401(k) Manager", desc: "Calculate and maximize your employer and employee contributions based on your exact net income." },
              { icon: BarChart3,  color: "primary",  title: "Tax Optimizer", desc: "Stay ahead of IRS deadlines with accurate quarterly estimated tax calculations and reminders." },
              { icon: Activity,   color: "accent",   title: "Financial Dashboard", desc: "A clean, clinical snapshot of your net worth, upcoming liabilities, and financial health." },
            ].map(({ icon: Icon, color, title, desc }) => (
              <div key={title} className="bg-white p-8 rounded-2xl border border-gray-100 transition-transform hover:-translate-y-1 hover:shadow-md">
                <div className={`h-12 w-12 rounded-xl bg-${color}/10 flex items-center justify-center mb-6`}>
      ...
      <section className="py-24 bg-white border-y overflow-hidden">
        <div className="container mx-auto px-4 md:px-6">
          <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
            ...
            <div className="flex items-center space-x-3 bg-gray-50 p-2 rounded-full shadow-sm border border-gray-200">
              <Sun className={`h-4 w-4 ${!isDarkMode ? "text-primary" : "text-gray-400"}`} />
              <Switch checked={isDarkMode} onCheckedChange={setIsDarkMode} data-testid="switch-theme" />
            ...
            <div className="flex overflow-x-auto pb-8 -mx-4 px-4 gap-6 snap-x snap-mandatory md:grid md:grid-cols-2 lg:grid-cols-4 md:overflow-visible md:pb-0 md:px-0 md:mx-0">
              {screenshots.map((shot, i) => (
                <div key={shot.id} className="flex justify-center snap-center shrink-0">
                  <motion.div key={shot.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: i * 0.08 }}>
      ...
      <section className="py-24 md:py-32 bg-gray-50">
        <div className="container mx-auto px-4 md:px-6 max-w-3xl">
          ...
          >
            ...
            <div className="space-y-4">
              <h2 className="text-2xl md:text-3xl font-bold tracking-tight text-foreground">Built by a physician, for physicians.</h2>
              ...
              <div className="pt-2 space-y-1">
                ...
                <p className="text-sm font-bold tracking-widest text-primary uppercase">CLINICAL AI · MEDICAL FINTECH</p>
              </div>
            </div>
          </motion.div>
        </div>
      ...
      <section className="py-24 bg-primary text-white text-center relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 md:px-6 max-w-3xl relative z-10">
          <h2 className="text-3xl md:text-5xl font-bold tracking-tight mb-8">Take control of your financial future</h2>
          <div className="flex flex-col items-center gap-2">
            ...
            <span className="text-white/60 text-sm">$9.99/mo after trial</span>
          </div>
        </div>
      </section>
